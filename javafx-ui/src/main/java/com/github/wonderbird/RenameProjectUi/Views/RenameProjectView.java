...
public class RenameProjectView implements FxmlView<RenameProjectViewModel>, Initializable
{
   ...
   public void initialize(final URL location, final ResourceBundle resources)
   {
      ...
      firstLowerThenCamelCaseFromLabel.textProperty()
                                      .bindBidirectional(viewModel.firstLowerThenCamelCaseFromProperty());
      ...
      enableOriginalReplacementCheckBox.selectedProperty()
                                       .bindBidirectional(viewModel.enableOriginalReplacementProperty());
      enableCamelCaseReplacementCheckBox.selectedProperty()
                                        .bindBidirectional(viewModel.enableCamelCaseReplacementProperty());
      enableFirstLowerThenCamelCaseReplacementCheckBox.selectedProperty()
                                                      .bindBidirectional(
                                                         viewModel.enableFirstLowerThenCamelCaseReplacementProperty());
      enableLowerCaseReplacementCheckBox.selectedProperty()
                                        .bindBidirectional(viewModel.enableLowerCaseReplacementProperty());
      enableUpperCaseReplacementCheckBox.selectedProperty()
                                        .bindBidirectional(viewModel.enableUpperCaseReplacementProperty());
      enableSpaceSeparatedReplacementCheckBox.selectedProperty()
                                             .bindBidirectional(viewModel.enableSpaceSeparatedReplacementProperty());
      enableDashSeparatedReplacementCheckBox.selectedProperty()
                                            .bindBidirectional(viewModel.enableDashSeparatedReplacementProperty());

      viewModel.subscribe(Notification.BROWSESTARTDIR.toString(), (key, payload) -> browseStartDir((String)payload[0]));
   }

   private void browseStartDir(final String initialDir)
   {
      final DirectoryChooser directoryChooser = new DirectoryChooser();
      directoryChooser.setInitialDirectory(new File(initialDir));
      final File file = directoryChooser.showDialog(fromTextField.getScene().getWindow());
      if(file != null)
      {
