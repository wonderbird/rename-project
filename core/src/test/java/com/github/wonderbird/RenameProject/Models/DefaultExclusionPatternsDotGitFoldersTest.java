...
public class DefaultExclusionPatternsDotGitFoldersTest {
    ...
    public static Collection<Object[]> data() {
        return Arrays.asList(new Object[][]{
                {MUST_MATCH, ".git"},
                {MUST_MATCH, ".git"},
                {MUST_MATCH, ".git/matches"},
                {MUST_MATCH, "something/.git/else matches"},
                {MUST_MATCH, ".git/matches"},
                {MUST_MATCH, "something/.git"},
                {MUST_MATCH, "something/.git/matches"},
    ...
    @Test
    public void test() {
        boolean result = Pattern.matches(DefaultExclusionPatterns.DOT_GIT_FOLDERS, input);

        String message = String.format("DOT_GIT_FOLDERS %s '%s'", isMatchExpected ? "must match" : "must not match", input);
