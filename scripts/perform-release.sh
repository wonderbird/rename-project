...
POST_CREATE_RELEASE_RESPONSE=$(curl -s --data "{\"tag_name\":\"$RELEASE_TAG\",\"target_commitish\":\"master\",\"name\":\"Automatic Release\",\"body\":\"This release has bee created automatically by the build server [Travis CI](https://travis-ci.org/$TRAVIS_REPO_SLUG/builds/$TRAVIS_BUILD_ID).\n\nCommit message: $TRAVIS_COMMIT_MESSAGE\",\"draft\":true,\"prerelease\":true}"" --header "authorization: bearer $GITHUB_ACCESS_TOKEN" https://api.github.com/repos/wonderbird/rename-project/releases)
